package se.kodapan.osm.data.planet.parser.xml.streaming;
 ...
 */
public class StreamingOsmXmlParser {

  private static final Logger log = LoggerFactory.getLogger(StreamingOsmXmlParser.class);
  ...
  public void read(Reader xml) throws Exception {
    ...
    while (xmlr.hasNext()) {
      ...
      if (eventType == XMLStreamConstants.START_ELEMENT) {
        ...
        if ("node".equals(xmlr.getLocalName())) {
          processParsedNode(currentNode);
          currentNode = null;
          current = null;

        } else if ("way".equals(xmlr.getLocalName())) {
          processParsedWay(currentWay);
          currentWay = null;
          current = null;

        } else if ("relation".equals(xmlr.getLocalName())) {
          processParsedRelation(currentRelation);
